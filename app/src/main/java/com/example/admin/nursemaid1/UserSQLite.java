...
/**
 * Created by 宜德 on 2019/1/3.
 */

public class UserSQLite extends SQLiteOpenHelper {                                                      //紀錄有誰使用過tag，用來記住校正碼資訊
    ...
    final static String TABLE_NAME_4 = "calibrationcode_table";							//儲存使用者有量測記錄的小時的資料表
    final static String FIELD_ID_4 = "id";										//編號
    final static String Address = "address";						//address
    final static String user_names = "username";						//name
    final static String Red_Code = "codered";						//校正紅
    final static String Blue_Code = "codeblue";						//校正藍
    final static String Value_Code = "codevalue";						//校正時間
    final static String Time = "time";						//校正時間
    ...
    private void ToNewUserTable(String name){   //時間Table
        String sql = "CREATE TABLE "+TABLE_NAME_4+"("+FIELD_ID_4+" INTEGER primary key autoincrement,"
                +Address+" VARCHAR,"+user_names+" VARCHAR,"+Red_Code+" VARCHAR,"+Blue_Code+" VARCHAR,"+Value_Code+" VARCHAR,"+Time+" text)";
...
//		MyDb.execSQL(sql);
    ...
    public boolean isHaveEver(String address, String Uname){       //找是否有使用過
        address = "'"+address+"'";
        Uname = "'"+Uname+"'";
        Cursor cursor = null;                                           //從SQLite類別裡抓回來的資料是 Cursor 物件
        SQLiteDatabase dbr = this.getReadableDatabase();
        String where = Address+" = "+address+" AND "+user_names+" = "+Uname;
        //Log.e("isHaveTimeData_where",where);
        cursor = dbr.query(TABLE_NAME_4, null, where, null, null, null, null);    //query() 查詢指定的資料
    ...
    public void ToInsert(String address, String Uname, String red, String blue, String val, String time){  //新增資料
        ContentValues cv = new ContentValues();
        SQLiteDatabase dbw = this.getWritableDatabase();
        if(isHaveEver(address,Uname)) {
            address = "'"+address+"'";
            Uname = "'"+Uname+"'";
            String where = Address+" = "+address+" AND "+user_names+" = "+Uname;
            cv.put(Red_Code, red);
            cv.put(Blue_Code, blue);
            cv.put(Value_Code, val);
            cv.put(Time, time);
            dbw.update(TABLE_NAME_4, cv, where, null);
        }else {
            cv.put(Address, address);
            cv.put(user_names, Uname);
            cv.put(Red_Code, red);
            cv.put(Blue_Code, blue);
            cv.put(Value_Code, val);
            cv.put(Time, time);
            dbw.insert(TABLE_NAME_4, null, cv);
        }
        dbw.close();
    }

    public boolean isHaveData(String address){                                                             //確認有沒有使用者
        address = "'"+address+"'";
        Cursor cursor = null;                                           //從SQLite類別裡抓回來的資料是 Cursor 物件
        SQLiteDatabase dbr = this.getReadableDatabase();
    ...
    public String ToGetNowUser(String address){                                             //取現在使用者名子
        Log.e("ToGetUser","=================================");
        String Uname = null ;
        ...
        SQLiteDatabase dbr = this.getReadableDatabase();
        String where = Address+" = "+"'"+address+"'";
        Log.e("ToGetTimeData6", where);

        cursor = dbr.query(TABLE_NAME_4, null, where, null, null, null, Time+" DESC");    //query() 查詢指定的資料

        for(int i=0;i<cursor.getCount();i++){
            cursor.moveToPosition(i);  //移到指定位置
            Uname = cursor.getString(2);  //add第二個欄位內容
            break;
        }
        Log.e("Nowname", String.valueOf(Uname));
        return Uname;
    }

    public ArrayList<String> ToGetAllUser(String address){                                             //取全部使用者名子
        Log.e("ToGetUser","=================================");
        ...
        for(int i=0;i<cursor.getCount();i++){
            cursor.moveToPosition(i);  //移到指定位置
            Uname.add(cursor.getString(2));  //add第二個欄位內容
        }
        Log.e("ToGetTimeData_al", String.valueOf(Uname));
        return Uname;
    }

    public ArrayList<String> ToGetdata(String address, String user){                                              //取校正碼
        Log.e("ToGetdata","=================================");
        ArrayList<String> data = new ArrayList<String>();

        Cursor cursor = null;
        SQLiteDatabase dbr = this.getReadableDatabase();
        String where = Address+" = "+"'"+address+"'"+" AND "+user_names+" = "+"'"+user+"'";
        cursor = dbr.query(TABLE_NAME_4, null, where, null, null, null, null);    //query() 查詢指定的資料

        for(int i=0;i<cursor.getCount();i++){
            cursor.moveToPosition(i);  //移到指定位置
            data.add(cursor.getString(3));  //add第二個欄位內容
            data.add(cursor.getString(4));  //add第二個欄位內容
            data.add(cursor.getString(5));  //add第二個欄位內容
            break;
        }
        Log.e("ToGetTimeData_al", String.valueOf(data));
        return data;
    }
